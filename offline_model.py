...
data = pd.read_csv("offline_data.csv", encoding='latin')
...
for model_name in ["offline_model_LR.pickle", "offline_model_SVM.pickle", "offline_model_KNN.pickle", "offline_model_RF.pickle"]:
    saved_model = open(model_name, "rb")
    model = pickle.load(saved_model)
    predictions = model.predict(x_test)
    print(model_name)

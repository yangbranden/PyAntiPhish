...
def calculate_metrics():
    ...
    for model_name in ["url3_model_LR.pickle", "url3_model_SVM.pickle", "url3_model_KNN.pickle", "url3_model_RF.pickle"]:
        if model_name == "url3_model_LR.pickle": # Logistic Regression
            ...
            features = np.array(data[["subdomain_len", "subdomain_len_ratio", "netloc_len", "pathcomp_len", "period_count", "slash_count", "dash_count", "equal_count", 
                "colon_count", "has_bad_tld", "has_tls"]])
...
def predict_url(url, model_selector):
    ...
    if model_selector == 0: # Logistic Regression
        ...
        target_url_data = [[
            subdomain_len, subdomain_len_ratio, netloc_len, pathcomp_len, period_count, slash_count, dash_count, equal_count, 
            colon_count, has_bad_tld, has_tls
        ]]
